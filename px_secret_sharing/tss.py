...
if sys.version_info < (3, 0):
    ...
    def byte_to_ord(x): return ord(x)
    basestring = basestring
    unicode = unicode
    bytes = str
    def digest_size(d): return d.digestsize
else:
    def b(x): return x.encode('iso8859-1')
    def byte_to_chr(x): return chr(x)
    def byte_to_ord(x): return x
    basestring = str
    unicode = str
    bytes = bytes
    def digest_size(d): return d.digest_size
